...
public class DateToolTest {
  ...
  public void testIntervalMillis() {
    ...
    Assert.assertTrue(newTime - currentTime >= interval);
  }
  ...
  public void testConvertNanosToMillis() {
    ...
    Awaitility.await().pollDelay(RandomUtils.nextLong(1, 100), TimeUnit.MILLISECONDS).until(() -> true);
    Awaitility.await().pollDelay(RandomUtils.nextLong(1, 100), TimeUnit.NANOSECONDS).until(() -> true);
    ...
    Assert.assertTrue(((long) DateTool.convertNanosToMillis(intervalNanos) == intervalMillis + 1)
